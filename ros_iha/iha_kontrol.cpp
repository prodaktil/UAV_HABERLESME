...
void DroneController::printTelemetry()
{
    while (true)
    {
        ...
        ros1 = position.latitude_deg;
        ros2 = position.longitude_deg;
...
int DroneController::iha_k(int iha_deger){
iha_kon = iha_deger;
return 0;
}
int DroneController::ros_server()
{
  int argc ;
  char **argv;
  ros::init(argc, argv, "IHA_c");
  ros::NodeHandle n;
  ros::ServiceClient client = n.serviceClient<beginner_tutorials::AddTwoInts>("IHA_KONUM");
  ...
  if (client.call(srv))
  {
    ROS_INFO("X_konumu: %lf", (double)srv.response.xx);
    ROS_INFO("Y_konumu: %lf", (double)srv.response.yy);
  }
 std::cout <<"xxx; "<<srv.response.xx <<std::endl;
 std::cout <<"yyy; "<< srv.response.yy<<std::endl;
...
int DroneController::ros_server1()
{
  std::cout <<"ros a veri gonderiliyor"<< std::endl;
  int argc ;
  char **argv;
  ros::init(argc, argv, "IHA_c");
  ros::NodeHandle n;
...
int DroneController::ros_server2()
{
 std::cout <<"ros'tan veri aliyor "<< std::endl;
  int argc ;
  char **argv;
  ros::init(argc, argv, "IHA_c");
  ros::NodeHandle n;
